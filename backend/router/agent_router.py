...
def agents_list(request: Request):
    ...
    db.close()
    return templates.TemplateResponse('dashboard/agents/agents.html', {'request': request, 'agents': agents, 'agent': '', 'permission': request.cookies.get('Permission'), 'language': eval(request.cookies.get('UserLang')), 'menu': eval(request.cookies.get('Menu'))})
...
def agents_view(request: Request, agent_number: str):
    ...
    if permission == 'super':
        ...
        db.close()
        return templates.TemplateResponse('dashboard/agents/agents_view.html', {'request': request, 'agents': agents, 'agent': agent, 'permission': permission, 'language': eval(request.cookies.get('UserLang')), 'menu': eval(request.cookies.get('Menu'))})
...
def agents_queries(request: Request, agent_number: str):
    ...
    db.close()
    return templates.TemplateResponse('dashboard/agents/agents_queries.html', {'request': request, 'agents': agents, 'agent_queries': agent_queries, 'agent_number': agent_number, 'permission': request.cookies.get('Permission'), 'language': eval(request.cookies.get('UserLang')), 'menu': eval(request.cookies.get('Menu'))})
...
async def agents_new(request: Request):
    ...
    if permission == 'super':
        ...
        agents = db.query(Agent).order_by(Agent.agent_name.asc()).all()
        db.close()
        return templates.TemplateResponse('dashboard/agents/agents_new.html', {'request': request, 'agents': agents, 'permission': permission, 'language': eval(request.cookies.get('UserLang')), 'menu': eval(request.cookies.get('Menu'))})
...
async def agents_new(request: Request):
    ...
    if permission == 'super':
        ...
        if agent_ws:
            ...
            agents = db.query(Agent).order_by(Agent.agent_name.asc()).all()
            return templates.TemplateResponse('dashboard/agents/agents_new.html', {'request': request, 'agents': agents, 'permission': permission, 'language': eval(request.cookies.get('UserLang')), 'menu': eval(request.cookies.get('Menu')), 'msg': msg})
...
async def agents_edit(request: Request, agent_number: str):
    ...
    if permission == 'super':
        ...
        agent = db.query(Agent).filter(Agent.agent_number == agent_number).first()
        db.close()
        return templates.TemplateResponse('dashboard/agents/agents_edit.html', {'request': request, 'agents': agents, 'agent': agent, 'agent_logeado': agent_logeado, 'permission': permission, 'language': eval(request.cookies.get('UserLang')), 'menu': eval(request.cookies.get('Menu'))})
        ...
        if agent_ws:
            ...
            return templates.TemplateResponse('dashboard/agents/agents_edit.html',
                                              {'request': request, 'agents': agents, 'agent': agent,
                                               ...
                                               'constants.alias_expert': constants.alias_expert.capitalize(), 'permission': permission, 'language': eval(request.cookies.get('UserLang')), 'menu': eval(request.cookies.get('Menu')), 'msg': msg})
        db.close()
