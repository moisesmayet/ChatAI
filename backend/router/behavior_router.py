...
def behaviors_list(request: Request):
    ...
    db.close()
    return templates.TemplateResponse('dashboard/behaviors/behaviors.html', {'request': request, 'behaviors': behaviors, 'permission': request.cookies.get('Permission'), 'language': constants.language})
...
def behaviors_view(request: Request, behavior_code: str):
    ...
    if permission == 'super':
        ...
        db.close()
        return templates.TemplateResponse('dashboard/behaviors/behaviors_view.html', {'request': request, 'behaviors': behaviors, 'behavior': behavior, 'permission': permission, 'language': constants.language})
...
async def behaviors_new(request: Request):
    ...
    if permission == 'super':
        ...
        behaviors = db.query(Behavior).order_by(Behavior.behavior_code.asc()).all()
        db.close()
        return templates.TemplateResponse('dashboard/behaviors/behaviors_new.html', {'request': request, 'behaviors': behaviors, 'permission': permission, 'language': constants.language})
...
async def behaviors_edit(request: Request, behavior_code: str):
    ...
    if permission == 'super':
        ...
        behavior = db.query(Behavior).filter(Behavior.behavior_code == behavior_code).first()
        db.close()
        return templates.TemplateResponse('dashboard/behaviors/behaviors_edit.html', {'request': request, 'behaviors': behaviors, 'behavior': behavior, 'permission': permission, 'language': constants.language})
...
async def behaviors_delete(request: Request, behavior_code: str):
    ...
    if permission == 'super':
        ...
        behavior = db.query(Behavior).filter(Behavior.behavior_code == behavior_code).first()
        db.close()
        return templates.TemplateResponse('dashboard/behaviors/behaviors_delete.html', {'request': request, 'behaviors': behaviors, 'behavior': behavior, 'permission': permission, 'language': constants.language})
